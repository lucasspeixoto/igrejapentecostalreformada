...
describe('AuthenticationService (vitest)', () => {
  ...
  it('getSessionFromStorage should return null when no session in localStorage', () => {
    vi.spyOn(Storage.prototype, 'getItem').mockReturnValue(null);
    expect(service.getSessionFromStorage()).toBeNull();
  });

  it('getSessionFromStorage should read currentSession from stored json', () => {
    const stored = JSON.stringify({ currentSession: { access_token: 'a' } });
    vi.spyOn(Storage.prototype, 'getItem').mockReturnValue(stored);

    const session = service.getSessionFromStorage();

    expect(session).toEqual({ access_token: 'a' });
  });

  it('getSessionFromStorage should fallback to session from stored json', () => {
    const stored = JSON.stringify({ session: { access_token: 'b' } });
    vi.spyOn(Storage.prototype, 'getItem').mockReturnValue(stored);

    const session = service.getSessionFromStorage();

    expect(session).toEqual({ access_token: 'b' });
  });
});
